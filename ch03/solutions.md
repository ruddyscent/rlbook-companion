...
세 예시는 MDP 틀이 성격이 다른 문제들에도 두루 적용됨을 드러낸다. 창고 로봇은 구체적인 물리 제어 문제, 치료 계획은 불확실성 속의 중대한 순차적 의사결정 문제, 이야기 생성 도우미는 상태와 보상의 정의가 까다로운 주관적 상호작용 문제이다.
...
이 식은 두 단계로 평균을 취하는 구조다. 안쪽의 이중 합 $\sum_{s', r} r \cdot p(s', r \mid s, a)$는 상태 $s$에서 행동 $a$를 취했을 때의 기대 보상 $r(s, a)$에 해당하며, 바깥의 합은 이 행동별 기대 보상을 정책 확률 $\pi(a \mid s)$로 가중평균한다. 즉 "행동을 어떻게 고를지"와 "그 행동의 결과가 어떻게 나올지"라는 두 단계의 불확실성이 차례대로 평균화된다.
...
이다. 상태 $s$에서 행동 $a$를 취하면 다음 상태 $s'$와 보상 $r$이 $p(s', r \mid s, a)$의 분포에 따라 결정되고, 다음 상태 $s'$에서부터는 정책 $\pi$를 따르게 되므로 그 이후의 기대 이득은 정의에 의해 $v_\pi(s')$와 같다. 이를 정리하면 다음과 같다.
...
을 자연스럽게 얻을 수 있다. 즉 두 연습문제는 단순한 항등식처럼 보이지만, 실제로는 강화학습 전반에 등장하는 벨만 방정식의 두 구성 요소를 분리해 보여 준다.
...
이다. 상태 $s$에서 행동 $a$를 취하면 환경의 동역학 $p(s', r \mid s, a)$에 따라 다음 상태 $s'$와 보상 $r$이 결정되고, 이후 상태 $s'$에서는 정책 $\pi$에 따라 다음 행동 $a'$가 선택되며 그 이후의 기대 이득은 $q_\pi(s', a')$이다. 이를 펼쳐 쓰면 다음과 같다.
...
최적 정책은 행동 가치를 최대화하는 행동을 확률 $1$로 선택하므로 이 등식이 성립한다. [Exercise 3.18](#exercise-3-18)의 정책 평균이 결정적 최댓값으로 바뀐 형태로도 이해할 수 있다.
