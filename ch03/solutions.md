...
을 얻는다. 표에 표시된 $v_\pi$는 소수 첫째 자리까지만 정확하다고 명시되어 있으므로 $0.675 \approx 0.7$로 반올림되어 중앙값과 일치한다. 따라서 그림에 주어진 숫자들을 직접 대입했을 때 벨만 방정식의 좌변과 우변이 (반올림 오차 범위 안에서) 같다는 의미에서 중앙 상태에서 벨만 방정식이 수치적으로 성립한다.
