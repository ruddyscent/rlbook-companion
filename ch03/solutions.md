...
[예제 3.5](notes.md#example-3-5)의 그림 3.2(오른쪽)에 나타난 가치함수 $v_\pi$는 균등 무작위 정책($\pi(a \mid s) = 0.25$, 네 가지 행동 각각)과 할인율 $\gamma = 0.9$ 아래에서 계산된 값이다. 중앙 상태의 값은 $+0.7$이고, 그 네 이웃(북·동·남·서)의 값은 $+2.3,\ +0.4,\ -0.4,\ +0.7$이다. 중앙 상태에서는 어떤 방향으로 이동하더라도 격자 안에 머물고 특수 상태(A, B)도 아니므로 즉시 보상은 모두 $0$이고, 전이는 결정적이다.
...
이 된다. 백업 다이어그램에서 루트 $(s, a)$로부터 각 다음 상태 $s'_1, s'_2, s'_3$로 뻗는 가지의 보상이 $r_i$, 가지 확률이 $p(s', r \mid s, a)$, 자식 노드의 값이 $v_\pi(s')$이므로 두 식이 다이어그램과 정확히 대응한다. 이 식과 [Exercise 3.18](#exercise-3-18)의 식을 합치면 [Exercise 3.17](#exercise-3-17)의 $q_\pi$에 대한 벨만 방정식을 다시 얻는다.
...
격자 세계 예제([예제 3.5](notes.md#example-3-5))에서 최선 상태는 특수 상태 A이다. A에서는 어떤 행동을 선택해도 보상 $+10$과 함께 A'으로 이동하고, A'에서는 통상의 격자 이동 규칙을 따른다. 최적 정책에서 A'에 도달한 뒤에는 가능한 한 빠르게 A로 돌아가야 하며, A'은 A로부터 네 칸 아래에 있으므로 위쪽으로 네 번 연속 이동하는 것이 최적이다. 이 동안의 보상은 모두 $0$이다.
...
이는 최적 정책이 항상 행동 가치를 최대화하는 행동을 (확률 $1$로) 선택하기 때문이다. [Exercise 3.18](#exercise-3-18)의 정책 평균이 결정적 최댓값으로 바뀐 형태로도 이해할 수 있다.
...
이 식과 [Exercise 3.25](#exercise-3-25)의 식을 결합하면 $v_*$에 대한 벨만 최적 방정식
