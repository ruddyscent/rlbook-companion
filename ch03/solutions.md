...
와 같이 쓸 수 있다. 즉 어떤 상태 $s$에서 행동 $a$를 취했을 때 가능한 모든 다음 상태와 보상에 대한 확률의 합이 $1$이라는 사실을 나타낸다.
...
보상 $R_{t+1}$은 두 단계의 확률적 과정을 거쳐 결정된다. 먼저 상태 $s$에서 정책 $\pi(a \mid s)$에 따라 행동 $a$가 선택되고, 이어서 환경의 동역학 $p(s', r \mid s, a)$에 의해 다음 상태 $s'$와 보상 $r$이 동시에 결정된다. 따라서 기댓값은 가능한 모든 행동 $a$, 다음 상태 $s'$, 보상 $r$에 대한 합으로 쓸 수 있다.
...
이다. 상태 $s$에서 행동 $a$를 취하면 다음 상태 $s'$와 보상 $r$이 $p(s', r \mid s, a)$의 분포에 따라 결정되고, 다음 상태 $s'$에서부터는 정책 $\pi$를 따르게 되므로 그 이후의 기대 이득은 정의에 의해 $v_\pi(s')$와 같다. 이를 정리하면 다음 식을 얻는다.
...
을 자연스럽게 얻을 수 있다. 즉 두 연습문제는 단순한 항등식처럼 보이지만, 실제로는 강화학습 전반에 등장하는 벨만 방정식의 두 구성 요소를 분리해 보여 주는 셈이다.
...
을 얻는다. 표에 표시된 $v_\pi$는 소수 첫째 자리까지만 정확하다고 명시되어 있으므로 $0.675 \approx 0.7$로 반올림되어 중앙값과 일치한다. 따라서 중앙 상태에서 벨만 방정식이 수치적으로 성립한다.
...
를 얻는다. 이 식이 곧 $q_\pi$에 대한 벨만 방정식이며, 문제에서 제시된 백업 다이어그램에서 가지 $p$는 환경의 전이를, 가지 $\pi$는 다음 상태에서의 정책에 따른 행동 선택을 나타낸다.
...
이 된다. 백업 다이어그램에서 루트 $(s, a)$로부터 각 다음 상태 $s'_1, s'_2, s'_3$로 뻗는 가지의 보상이 $r_i$, 가지 확률이 $p(s', r \mid s, a)$, 자식 노드의 값이 $v_\pi(s')$이므로 두 식이 다이어그램과 정확히 대응한다. 이 식과 Exercise 3.18의 식을 합치면 Exercise 3.17의 $q_\pi$에 대한 벨만 방정식을 다시 얻는다.
